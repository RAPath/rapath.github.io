---
title: Useful Links
sidebar_position: 2
description: Key official links for Russia medical device regulation — Roszdravnadzor, GRLS, Ministry of Health, EAEU, and legislation portals.
---

# Useful Links

## Russian regulatory authorities

| Resource | URL |
|---|---|
| **Roszdravnadzor (RZN)** | [roszdravnadzor.gov.ru](https://roszdravnadzor.gov.ru/) |
| **Ministry of Health (Minzdrav)** | [minzdrav.gov.ru](https://minzdrav.gov.ru/) |
| **GRLS — State Register of Medical Devices** | [grls.rosminzdrav.ru/GRLS.aspx](https://grls.rosminzdrav.ru/GRLS.aspx) |
| **FGBU National Institute of Quality** | [rosminzdrav.ru](https://rosminzdrav.ru/) |
| **Rosstandart (GOST R standards)** | [gost.ru](https://www.gost.ru/) |

## EAEU

| Resource | URL |
|---|---|
| **Eurasian Economic Commission (EEC)** | [eec.eaeunion.org](https://eec.eaeunion.org/) |
| **EAEU medical device database** | [eec.eaeunion.org](https://eec.eaeunion.org/) |

## Legislation portals

| Resource | URL |
|---|---|
| **Garant legal database** | [base.garant.ru](https://base.garant.ru/) |
| **Consultant Plus** | [consultant.ru](https://www.consultant.ru/) |
| **Official legal information portal** | [pravo.gov.ru](http://pravo.gov.ru/) |

## International

| Resource | URL |
|---|---|
| **IMDRF** | [imdrf.org](https://www.imdrf.org/) |
| **ISO standards** | [iso.org](https://www.iso.org/) |

## RAPath

| Resource | URL |
|---|---|
| **RAPath hub** | [rapath.org](https://rapath.org/) |
| **Community discussions (GitHub)** | [github.com/RAPath/hub/discussions](https://github.com/RAPath/hub/discussions) |
| **Contribute or report issues** | [github.com/RAPath/hub](https://github.com/RAPath/hub) |

:::info Disclaimer
Link accuracy is maintained on a best-effort basis. Official Russian government websites occasionally change structure. If a link is broken, search for the authority name directly.
:::

Add clarification note after the FGBU entry:

Note: The FGBU National Institute of Quality operates under Rosminzdrav. Specific FGBU contact details and submission procedures are typically communicated by Roszdravnadzor during the registration process.

Add a new bullet point after the FGBU entry:

Note: The FGBU National Institute of Quality operates under Rosminzdrav and conducts expert reviews on behalf of Roszdravnadzor. Specific FGBU contact details and submission procedures are typically communicated by Roszdravnadzor during the registration process. Direct contact with FGBU is not required — all submissions go through Roszdravnadzor.
