...
description: What field safety corrective actions (FSCAs) are, how they differ from recalls, and how to manage them under Australia's PRAC framework.
sidebar_position: 3
...
**Product Correction (most common FSCA type)**
Where the device can remain in use but a correction is needed — for example:
...
**Recall**
Where the device must be removed from use or from the market — for example:
...
**Product Alert**
Where users need to be notified of a potential issue, but the device can continue to be used with appropriate precautions, without requiring product return or correction.
...
1. Notify the TGA Recalls team as soon as they become aware of the overseas action
2. Assess whether Australian-supplied devices are affected (same batch, same manufacturing period, same component)
3. Initiate an equivalent Australian market action if Australian devices are affected
4. Not assume that an overseas FSCA automatically applies to Australian devices — nor assume it doesn't
...
3. **TGA reviews and approves** the strategy (or requests modifications)
4. **Execute the market action** — issue FSN, implement corrective actions, track responses
5. **Submit a close-out report** to the TGA confirming the action is complete
...
- [Recalls and other market actions — TGA](https://www.tga.gov.au/safety/recalls-and-other-market-actions)
