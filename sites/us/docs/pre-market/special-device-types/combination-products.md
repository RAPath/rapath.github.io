---
sidebar_position: 3
title: Combination Products
description: FDA regulation of drug-device, biologic-device, and drug-biologic-device combination products — PMOA, OCP jurisdiction, and regulatory requirements.
keywords: [combination products FDA, drug device combination, 21 CFR Part 3, OCP, PMOA]
---

# Combination Products

See [Combination Products — 21 CFR Part 3](/docs/pre-market/classification/combination-products) for the full overview of the combination product framework.

This page covers practical regulatory requirements for common combination product types.

## Drug-eluting devices (CDRH lead)

For drug-eluting stents, drug-coated catheters, and similar products where the device PMOA predominates:
- Submit to **CDRH** via PMA (typically Class III)
- Must comply with **21 CFR Part 820 (QMSR)** for the device component
- Drug component manufacturing must comply with **21 CFR Part 211** (cGMPs)
- CDRH consults with CDER on the drug component review

## Prefilled drug delivery devices (CDER lead)

For prefilled syringes, autoinjectors, and delivery devices where the drug PMOA predominates:
- Submit to **CDER** (NDA, BLA, or ANDA)
- Device component safety evaluated as part of the drug application
- CDER consults with CDRH on the device component

## Official resources
- [FDA OCP — Combination Products](https://www.fda.gov/combination-products)
- [21 CFR Part 3](https://www.ecfr.gov/current/title-21/chapter-I/subchapter-A/part-3)

Primary Mode of Action (PMOA) Determination

For combination products, the lead center is determined by identifying the primary mode of action:

CDRH leads if: The primary mode of action is device-based (the drug/biologic component contributes to the overall effect but the device mechanism is primary)

CDER leads if: The primary mode of action is drug or biologic-based (the drug/biologic mechanism is primary, and the device is ancillary)

When PMOA is unclear, FDA's Office of Combination Products (OCP) can provide guidance through a Combination Product Classification Request.

**Requesting a PMOA Determination**

When the primary mode of action is unclear, sponsors should submit a Combination Product Classification Request to FDA's Office of Combination Products (OCP). OCP typically responds within 30 days of a complete request. This determination should be obtained before finalizing your premarket submission strategy, as it directly determines which center (CDRH or CDER) will lead the review and what regulatory pathway applies.
