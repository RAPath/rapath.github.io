---
title: Combination Products
sidebar_position: 5
description: "Drug-device and device-biologic combination products under China's regulatory framework."
keywords: [combination product,drug device,NMPA,China]
tags: [pre-market,special-types]
---

# Combination Products

*This page provides a plain English reference for combination products requirements under China's NMPA regulatory framework. Full content to be completed during editorial review.*

---

## Regulatory basis

Requirements for combination products are set out in:
- **MDSAR 2021** (State Council Order No. 739)
- Relevant NMPA technical guidelines published on [www.cmde.org.cn](https://www.cmde.org.cn/)
- Applicable YY/T and GB standards

---

## Classification implications

Device type significantly affects classification. Consult the [National Medical Device Classification Catalogue (NMDCC)](/cn/docs/pre-market/classification/nmdcc) for the applicable class for your specific product.

---

## Key obligations

Refer to [CMDE technical review guidelines](https://www.cmde.org.cn/) specific to this device type for the latest dossier requirements.

:::tip Official Sources
- **NMPA** — [www.nmpa.gov.cn](https://www.nmpa.gov.cn/)
- **CMDE** — [www.cmde.org.cn](https://www.cmde.org.cn/)
- **NMPA Medical Device Registration Query** — [datasearch.nmpa.gov.cn](https://www.nmpa.gov.cn/datasearch/home-index.html)
:::

:::note Disclaimer
Content on this site is written with AI assistance and is intended as a navigation aid only. Always verify against official [NMPA](https://www.nmpa.gov.cn/) sources before making regulatory decisions. Not affiliated with NMPA or any Chinese Government body. Not legal or regulatory advice.
:::

For combination products (devices combined with drugs or biological products), classification and review requirements depend on the dominant component. If the device is dominant, NMPA/CMDE conducts device review with coordination from the relevant drug authority. If the drug/biological component is dominant, the drug authority leads review. The dossier must demonstrate safety and efficacy of the combination as a single product, with bridging data showing interactions between components. Consult NMPA early to confirm which authority will lead the review.

Expand the 'Key obligations' section to include: 'For combination products (devices combined with drugs or biological products), classification and review requirements depend on the dominant component. If the device is dominant, NMPA/CMDE conducts device review with coordination from the relevant drug authority. If the drug/biological component is dominant, the drug authority leads review. The dossier must demonstrate safety and efficacy of the combination as a single product, with bridging data showing interactions between components. Consult NMPA early to confirm which authority will lead the review.'

Replace the placeholder text with a complete 'Regulatory pathway' section: 'Regulatory pathway\n\nFor combination products (devices combined with drugs or biological products):\n- Classification and review requirements depend on the dominant component\n- If the device is dominant, NMPA/CMDE conducts device review with coordination from the relevant drug authority\n- If the drug/biological component is dominant, the drug authority leads review\n- The dossier must demonstrate safety and efficacy of the combination as a single product, with bridging data showing interactions between components\n\nConsult NMPA early to confirm which authority will lead the review.'
