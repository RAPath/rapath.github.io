---
id: equivalence-claims
title: Equivalence claims
sidebar_label: Equivalence claims
sidebar_position: 3
tags: [mdr, clinical-evidence, equivalence, clinical-evaluation]
---
...
Equivalence is a **tool for accessing clinical data** — it does not reduce or eliminate the clinical evaluation requirement. The manufacturer must still conduct a rigorous clinical evaluation; they are simply using data from an equivalent device as a significant data source.

---

## The three equivalence criteria — all must be met

MDR Annex XIV §3 requires that all three of the following criteria are simultaneously met:
...
Clinical equivalence is the most outcome-focused criterion — is the device going to be used in the same way for the same patients in the same clinical context?
...
## Access to the equivalent device's technical documentation
...
For devices from the **same manufacturer** (e.g. a new generation of the manufacturer's own legacy device), access to technical documentation is inherent — no formal contractual arrangement is needed, but the documentation must exist and be retrievable.

---

## Equivalence for Class III and implantable Class IIb devices
...
## Characterising differences between devices
...
## Equivalence vs. general literature

Equivalence is distinct from general literature review:

| Type of evidence | What it supports |
|---|---|
| Clinical data from the **device itself** | Direct evidence — strongest |
| Clinical data from an **equivalent device** (with access to tech doc) | Indirect evidence — valid under MDR if criteria met |
| **General literature** on the device type or clinical area | Context and supporting evidence — not a substitute for device-specific data |
...
## Documenting the equivalence claim in the CER

The CER must include:
1. **Identification of the equivalent device**: manufacturer, device name, model, regulatory status
2. **Assessment against all three criteria**: clinical, technical, and biological — with evidence for each
3. **Access to technical documentation**: confirmation of access (or contractual arrangement)
4. **Characterisation of differences**: documented differences and justification that they do not affect safety/performance
5. **Conclusion**: whether full equivalence is established for each claim being supported
...
|---|---|
| MDR Art. 61(4)–(5) | Equivalence requirements |
