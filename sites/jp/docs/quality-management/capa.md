...
1. **Identify sources of CAPA inputs** — complaints, adverse event reports, internal audits, process monitoring data, management review outputs, nonconforming product reports
2. **Investigate root cause** — determining the actual root cause, not just the symptom
3. **Define the corrective/preventive action** — actions address root cause, not just the immediate occurrence
4. **Implement the action** — with defined timelines and responsible parties
5. **Verify effectiveness** — confirming the action has resolved the root cause and has not introduced new risks
6. **Document the process** — CAPA records must be maintained and retrievable for PMDA inspection
