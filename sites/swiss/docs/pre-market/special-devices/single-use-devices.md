---
sidebar_position: 9
description: Single-use medical devices in Switzerland — prohibition on reprocessing, labelling requirements, and Swissmedic guidance on SUD reprocessing.
---
# Single-use Devices

## Overview
A **single-use device** is a device intended to be used on one individual during a single procedure. MedDO prohibits the reprocessing and reuse of single-use devices. Manufacturers must clearly indicate single-use status in labelling. The ISO 7000-1051 single-use symbol must be displayed on device packaging.

## Prohibition on Reprocessing
MedDO Art. 17 prohibits reprocessing of single-use devices in Switzerland unless specific conditions are met. Healthcare institutions that wish to reprocess single-use devices for use in their own patients must meet the requirements of MedDO Art. 17 para. 2 and provide advance written notification to Swissmedic detailing their reprocessing procedures and quality assurance measures.

## Labelling Requirements
The manufacturer must: indicate single-use status prominently on the label; include the ISO 7000-1051 "do not reuse" symbol; provide information in the IFU explaining why reprocessing is not permitted (where this is not self-evident).


IFU Justification for Reprocessing Prohibition

Where the reason for prohibiting reprocessing is not immediately apparent to healthcare users, manufacturers must provide clear technical or clinical justification in the Instructions for Use. This justification should explain: material degradation risks; safety implications of reuse; validation data supporting single-use designation; potential impacts on device sterility, structural integrity, or performance.

## Official Sources
- [MedDO Art. 17 (SR 812.213)](https://www.fedlex.admin.ch/eli/cc/2021/205/en)


:::warning Disclaimer
AI-assisted content for navigation only. Always verify against official [Swissmedic](https://www.swissmedic.ch) and [Fedlex](https://www.fedlex.admin.ch) sources. Not legal or regulatory advice.
:::

Healthcare Institution Reprocessing

Under MedDO Art. 17 para. 2, healthcare institutions may reprocess single-use devices for use in their own patients only if they: establish and maintain a validated reprocessing procedure; implement appropriate quality assurance and risk management; maintain records of all reprocessing activities; notify Swissmedic of their reprocessing practices. The institution assumes full responsibility for the reprocessed device's safety and performance. Reprocessed devices must not be distributed to other healthcare facilities or the public.
