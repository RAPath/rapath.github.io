---
title: Device Grouping & Variants
sidebar_position: 7
description: MDA rules on grouping devices under a single registration, managing product variants, and size and configuration families in Malaysia.
---
...
1. The devices share the **same intended purpose**
2. The devices are manufactured by the **same manufacturer** at the same site (or sites covered by the same QMS)
3. The devices are of the **same generic type** (same design principle, same materials, same mode of action)
4. Differences between variants are limited to:
...
1. **Device identification table** — list all model/catalogue numbers, sizes, and configurations covered
2. **Variant justification** — explain how each variant differs from the reference/representative model
3. **Worst-case approach** — demonstrate conformity assessment for the "worst case" variant (e.g. largest size, highest energy, most complex configuration)
4. **Risk assessment coverage** — confirm the risk assessment covers all variants
5. **Labelling for all variants** — include labels for all included models
