---
title: Useful Links & Official Resources
sidebar_position: 1
description: Links to official SFDA portals, guidance documents, and key resources for medical device regulation in Saudi Arabia.
---

# Useful Links & Official Resources

## SFDA official portals

| Resource | Description |
|---|---|
| [SFDA Website](https://www.sfda.gov.sa/) | Official SFDA website — regulatory news, announcements, guidance |
| [GHAD Portal](https://ghad.sfda.gov.sa/) | Unified electronic system for MDMA, MDEL, UDI submissions |
| [SFDA Medical Devices Sector](https://www.sfda.gov.sa/en/medicaldevices) | Medical devices regulatory section |
| [NCMDR](https://www.sfda.gov.sa/en/ncmdr) | National Centre for Medical Device Reporting — adverse event submission |

## SFDA guidance documents (MDS-REQ series)

MDS-REQ and MDS-G guidance documents are available for download from the SFDA website under:
**Medical Devices → Regulations and Guidance → Guidance Documents**

Key documents to download:
- MDS-REQ 1 — Requirements for MDMA (core registration document)
- MDS-REQ 5 — Post-Market Surveillance and Vigilance
- MDS-REQ 7 — UDI requirements
- MDS-G008 — Classification guidance

## International reference resources

| Resource | Description |
|---|---|
| [IMDRF](https://www.imdrf.org/) | International Medical Device Regulators Forum |
| [ISO 13485:2016](https://www.iso.org/standard/59752.html) | QMS standard for medical devices |
| [ISO 14971:2019](https://www.iso.org/standard/72704.html) | Risk management for medical devices |
| [IEC 62304](https://webstore.iec.ch/publication/6792) | Software lifecycle for medical devices |
| [GS1 UDI](https://www.gs1.org/standards/udi) | GS1 UDI issuing agency |

## RAPath suite

- [RAPath Hub](https://rapath.github.io/) — all jurisdiction sites
- [Australia (TGA)](https://rapath.github.io/au/)
- [European Union (MDR/IVDR)](https://rapath.github.io/eu/)
- [Canada (Health Canada)](https://rapath.github.io/hc/)
- [Singapore (HSA)](https://rapath.github.io/sg/)
- [United Kingdom (MHRA)](https://rapath.github.io/uk/)
- [United States (FDA)](https://rapath.github.io/us/)

:::info Disclaimer
External links are provided for reference only. RAPath does not control the content of external sites. Always verify that you are using the most current version of any SFDA guidance document.
:::

## Key documents to download
- MDS-REQ 3 — QMS requirements and ISO 13485 certification

Add to the 'Key documents to download' list in 'SFDA guidance documents (MDS-REQ series)' section:
- MDS-REQ 3 — QMS requirements and ISO 13485 certification
